...
class Window:
    ...
    def create_test_mode_widgets(self):
        ...
        # Button for plotting eps(x)
        self.plot_last_column_button = tk.Button(self.master, text="График eps1(x)", command=self.plot_eps_x)
        self.plot_last_column_button.grid(row=2, column=2, sticky='we', padx=5, pady=5)
    ...
    def create_main_mode_widgets(self):
        ...
        # Button for plotting eps(x)
        self.plot_last_column_button = tk.Button(self.master, text="График eps2(x)", command=self.plot_eps_x)
        self.plot_last_column_button.grid(row=2, column=2, sticky='we', padx=5, pady=5)
    ...
    def plot_eps_x(self):
        try:
            ...
            new_window.title(f"График \u03B5{num}(n)")
            new_window.geometry("800x600")  # Фиксированный размер окна

            # Создание фигуры для нового графика
            fig = Figure(figsize=(6, 4), dpi=100)
            ax = fig.add_subplot(111)

            arr_eps = [0.] * 12
            arr_n = [10, 20, 50, 80, 100, 200, 500, 800, 1000, 10000, 100000, 500000]
            arr_n_draw = [10, 20, 50, 80, 100, 200, 500, 800, 1000, 10000, 100000, 1000000]
            for i, n in enumerate(arr_n):
                self.sol = Solver()
                self.sol.Solve(n, self.solve_mode)
                # print(n, self.sol.get_max_diff())
                arr_eps[i] = self.sol.get_max_diff()
            ...
            for i, el in enumerate(arr_eps):
                print(arr_n_draw[i], el)

            # Построение графика
            ax.plot(arr_n_draw, arr_eps, color='green', alpha=0.7)
            ax.set_xscale('log')  # Логарифмическая шкала для оси X
            ax.set_yscale('linear')
            ax.set_xlabel('n')
            ax.set_ylabel(f"\u03B5{num}(n)")
            # ax.legend()
            ax.grid()

            # Применение плотного макета
            fig.tight_layout()

            # Добавление графика в новое окно
            canvas = FigureCanvasTkAgg(fig, master=new_window)
            canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            canvas.draw()

            # Добавление кнопки закрытия
            btn_close = tk.Button(new_window, text="Закрыть", command=new_window.destroy)
